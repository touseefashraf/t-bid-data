...
export class OthersComponent implements OnInit {
    ...
    ngOnInit() {
        if (this.api.user.user_type == 'customer') {
            this.checkErrors(this.ds.pId, 'other')
        }
    }
    ngAfterViewInit() {
    }
    ...
    downloadDocument(documentId: number, file: any) {
        this.ds.downloadDocument(file.project_id, 'other', documentId).subscribe((resp: any) => {
            const binaryData = []
    ...
    getData() {
        const index = this.ds.projectDetails.documents.findIndex(d => d.document_type == 'other')
        if (index > -1) {
            return false
        } else {
            return true
