...
export class SpecsComponent implements OnInit, AfterViewInit {
    ...
    ngOnInit() {
        if (this.api.user.user_type == 'customer') {
            this.checkErrors(this.ds.pId, 'spec')
        }
    }
    ngAfterViewInit() {
    }
    ...
    downloadFolder() {
        this.ds.downloadFolder(this.ds.pId, 'spec').subscribe((resp: any) => {
            const binaryData = []
            binaryData.push(resp)
            const downloadLink = document.createElement('a')
            downloadLink.href = window.URL.createObjectURL(new Blob(binaryData, { type: resp.type }))
    ...
    getData() {
        console.log(this.ds.projectDetails.documents)
        const index = this.ds.projectDetails.documents.findIndex(d => d.document_type == 'spec')
        if (index > -1) {
            return false
        } else {
            return true
